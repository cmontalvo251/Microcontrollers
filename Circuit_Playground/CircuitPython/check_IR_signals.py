...
##Ok let's plot these signals
plti = myplt.plottool(12,'Pulse','Magnitude','Up Button')
r = len(up_button)
rms_vec = []
for ri in range(0,r):
    plti.plot(up_button[ri],color='blue')
    ##Let's also compute the root mean square
    rms_vec.append(RMS(up_button[ri],up_button_avg))
    
##Let's also plot the average signal
plti.plot(up_button_avg,'k')

###Turns out they are almost the same. Let's plot the RMS for every signal
plt_rms = myplt.plottool(12,'Signal','RMS','RMS for Up Button')
plt_rms.plot(rms_vec)
print 'Max RMS = ',max(rms_vec)
...
##Ok let's plot these signals
plti = myplt.plottool(12,'Pulse','Magnitude','RIght Button')
r = len(right_button)
rms_vec_right = []
for ri in range(0,r):
    plti.plot(right_button[ri],color='blue')
    ##Let's also compute the root mean square
    rms_vec_right.append(RMS(right_button[ri],right_button_avg))
    
##Let's also plot the average signal
plti.plot(right_button_avg,'k')

###Turns out they are almost the same. Let's plot the RMS for every signal
plt_rms = myplt.plottool(12,'Signal','RMS','RMS for Right Button')
plt_rms.plot(rms_vec_right)
print 'RMS Max = ',max(rms_vec_right)

##Alright so a couple things. Let's plot the avg up button and right button
plt_compare = myplt.plottool(12,'Pulse','Magnitude','Comparison between Up and Right Button')
plt_compare.plot(up_button_avg,color='blue',label='Up Button')
plt_compare.plot(right_button_avg,color='red',label='Right Button')
plt_compare.legend()

##Ok so now the other thing to learn is what is the RMS between both buttons
print 'RMS Between Buttons = ',RMS(up_button_avg,right_button_avg)

##Ok wow. The RMS between buttons is 9000 so I think we can safely say if the button press is within
## a threshold of 1000 the button is probably right.
