...
float sensorvalue_SPEED,sensorvalue_ANGLE;
...
void setup() {
  ...
  GPS.begin(9600); //GPS_SIMPLE defaults to 9600 baud rate
...
void loop() {
  ...
  if (millis()/1000.0 > 0.2 + lastPrint) { //100 means it logs data at 10 Hz since it waits 100 ms or 0.1 seconds
    ...
    Serial.print(GPS.speed); Serial.print(" "); ///speeed
    ...
    sensorvalue_SPEED = analogRead(0); //Get a value between 0 and 1023 from the analog pin connected to the anemometer
    sensorvalue_ANGLE = analogRead(1);
    if (printSerial) {
      Serial.print(sensorvalue_SPEED);
      Serial.print(" ");
      Serial.print(sensorvalue_ANGLE);
      Serial.print(" ");
    }
    //Log to SD card
    logfile.print(sensorvalue_SPEED);
    logfile.print(" ");
    logfile.print(sensorvalue_ANGLE);
    logfile.print(" ");
      
    if (printSerial){
      Serial.print(GPS.day); Serial.print(" ");
      Serial.print(GPS.month); Serial.print(" ");
      Serial.print(GPS.year); Serial.print(" "); //GPS.year 
      Serial.print("\n");
    }
    logfile.print(GPS.day); logfile.print(" ");
    logfile.print(GPS.month); logfile.print(" ");
    logfile.print(GPS.year); logfile.print(" "); //GPS.year 
    logfile.print("\n");
    logfile.flush();
  }
}
