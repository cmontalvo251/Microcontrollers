...
#include <SD.h>
//#include <avr/sleep.h>
#include <Adafruit_BME280.h> //Uncommenting this leaves 461 bytes and code still works
...
void loop() {
  ...
  if (millis()/1000.0 > 0.2 + lastPrint) { //100 means it logs data at 10 Hz since it waits 100 ms or 0.1 seconds
    ...
    logfile.print(GPS.speed); logfile.print(" "); //speed
    logfile.print(GPS.angle); logfile.print(" "); //angle
    ...
    if (sensorVoltage <= 0.4){ 
      ...
      windSpeed = ((sensorVoltage - 0.4)*32.0/(2.0 - 0.4));
    }   
    //Convert mph to m/s
    ...
    if (printSerial){
      ...
      Serial.print(GPS.year); Serial.print(" "); //year Make sure to fix this in 2018
      Serial.print("\n");
    }
    ...
    logfile.print(GPS.year); logfile.print(" "); //year Make sure to fix this in 2018
    logfile.print("\n");
    logfile.flush();
  }
