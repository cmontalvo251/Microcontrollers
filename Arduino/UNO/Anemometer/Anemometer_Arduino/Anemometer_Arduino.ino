...
//Everything needed for The anemometer Sensor
float sensorvalue;
int numanemometer = 1; //Number of anemometer Sensors plugged in
int analogInPin[1]; //The # in brackets must be the same as the number of anemometer Probes
...
void setup() {
  ...
  //while (!Serial);
  for (int idx = 0;idx<numanemometer;idx++) {
    analogInPin[idx] = idx+1; //Anemometer sensor is hooked up to pins 1-4
  }
...
void loop() {
  ...
  if (millis()/1000.0 > 0.2 + lastPrint) { //100 means it logs data at 10 Hz since it waits 100 ms or 0.1 seconds
    ...
    Serial.print(-99); Serial.print(" "); ///speeed
    Serial.print(-99); Serial.print(" "); //angle removed
    ...
    logfile.print(-99); logfile.print(" "); //speed and angle are removed
    logfile.print(-99); logfile.print(" "); //from GPS_SIMPLE.h if you use Adafuit_GPS.h 
    //you can get GPS.speed and GPS.angle
    logfile.print(GPS.altitude); logfile.print(" ");

    //Get anemometer sensor readings and print to file
    for (int idx = 0;idx<numanemometer;idx++) {
      sensorvalue = analogRead(analogInPin[idx]); //Get a value between 0 and 1023 from the analog pin connected to the anemometer
      ...
      if (sensorVoltage <= voltageMin){ 
        ...
        windSpeed = ((sensorVoltage - voltageMin)*windSpeedMax/(voltageMax - voltageMin)); //m/s
      }   
    ...
    if (printSerial){
      ...
      Serial.print(17); Serial.print(" "); //GPS.year removed. Make sure to change
      //this in 2018
      Serial.print("\n");
    }
    logfile.print(GPS.day); logfile.print(" ");
    logfile.print(GPS.month); logfile.print(" ");
    logfile.print(17); logfile.print(" "); //GPS.year removed. Make sure to change this
    //in 2018
    logfile.print("\n");
    logfile.flush();
  }
