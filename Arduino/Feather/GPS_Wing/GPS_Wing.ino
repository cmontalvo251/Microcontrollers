...
//   Connect the GPS TX (transmit) pin to Digital 3 (Shield uses 8)
//   Connect the GPS RX (receive) pin to Digital 2 (Shield uses 7)
...
//Adafruit_GPS GPS(&mySerial); //This is for the shield on the UNO
//Adafruit_GPS GPS(&Serial3); //THE NALU SENSOR IS PLUGGED INTO SERIAL3. Make sure this is different for you.
Adafruit_GPS GPS(&Serial1); //The ECXA is in Serial1 and the GPS Wing
...
//Then you need to install the SAMD ADafruit boards.
...
void setup()  
{
  ...
  if (!SD.begin(4)) {      // if you're using an UNO, you can use this line instead
...
SIGNAL(TIMER0_COMPA_vect) {
  char c = GPS.read();
  if (GPSECHO)
    if (c) UDR1 = c; // writing direct to UDR1 is much much faster than Serial.print 
