...
void serialFloatPrint(float f) {
  ...
  for(int i=0; i<4; i++) {
    ...
    Serial2.print(c1); //does this output a command between prints?
    Serial2.print(c2); //otherwise line 75 on .pde won't work because their aren't any commas (Look up like 10 lines of code)
  }
}
...
void serialPrintFloatArr(float * arr, int length) {
  for(int i=0; i<length; i++) {
    ...
    Serial2.print(","); //The commas are here dude. 
  }
}
...
void loop()
{
  ...
  if (Serial2.available() and deltaTime > SEND_PERIOD) { 
    receive_message = Serial2.readStringUntil('\n');
    last_read = time_now;
    ...
    Serial2.println("\r\n");
    Serial2.flush(); //wait for outgoing data to go out
  }
}
