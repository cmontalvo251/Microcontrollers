...
sensorFusion::sensorFusion()
{
	...
	//In global.h the GLB_LOOP_TIME is 5000ul what is ul?
	//ul is an unsigned long constant
	//dT is a float so in this case
	//dT = 0.005 - which by the way is 200 Hz. **facepalm**.
	dT = GLB_LOOP_TIME/1000000.0f;

	//SFUS_COMPF_RP_TAU_MIN = 2.5
	//while SFUS_COMPF_RP_TAU_MAX = 10.0
	//rollPitchA_minimum = 0.998
	//rollPitchA_maximum = 0.995
...
bool sensorFusion::writeInitData(Vector3f accel, Vector3f gyros, Vector3f magne, Vector3f barom)
{
				...
				&& initBaromReadsCnt >= (SFUS_INIT_BAROM_DROPS + SFUS_INIT_BAROM_READS))
	{
		...
		float currentA = getTiltProportionalA();  //CurrentA can be anywhere from 0.995 to 0.998
...
void sensorFusion::writeData(Vector3f accel, Vector3f gyros, Vector3f magne, Vector3f barom)
{
	...
	//Remember that the readSensors routine runs first so this routine assumes you
	//have accel,gyros and magnetometer data
	...
	//So this line of code only get roll and pitch
	Vector3f accelEulertmp = getEulerAnglesFromAccel(accel);
	//This gets angular velocity in Earth frame
	...
	float currentA = getTiltProportionalA(); //So this gets the 0.995 or 0.998 depending on
	//the angle of the quad.
	//ok so these functions here essentially set the value from 0.995 or 0.998
	cf[ROLL].setTauViaA(currentA);
	cf[PITCH].setTauViaA(currentA);
	
	// filtering for roll and pitch
	//Alright so then we get the Euler angles from accelerometer and combine them with
	//the angular rate
	...
	//The yaw angle is fused using magnetometer data
	RPY.z = cf[YAW].getCombinedEstimation(getEulerYawFromMagne(magne), gyrosEulerRatestmp.z);
	
	// yaw is +/- Pi
	//this wrap_Pi function is inside the mathhelp.h routine
	RPY.z = wrap_Pi(RPY.z);
	...
	//By the way height is created using a mocing average filter
	//It takes the raw accelerometer data - integrates it to get heightdot while using a complimentary filter
	//on heightDot where the new measurement is a first order derivative filter and the integration variable is he accelerometer data
	//height is a moving average filter on barometer
...
{	
	// calculate the total tilt angle (pitch and roll)
        //This computes the total title angle in degrees
        //RPY.x and RPY.y are pitch and roll in radians
	float T_total = RAD2DEG*(acos(cos(RPY.x)*cos(RPY.y)));
	
	// limit the tilt angle
	//This limits the title angle from 0 to 30.0f degrees which is set in
	//the sensorFusion.h file
	T_total = constrainn<float>(T_total, 0.0f, SFUS_COMPF_RP_LIM_TILT);

	//Then we map
	//The xaxis is 0 to 30.0f
	//the yaxis is 0.998 to 0.995
	//So when the quad is on the ground it uses 0.998 but when it's titled to 30 degrees
	//it changes to 0.995 wierd. So when it tilts it filters more.
...
Vector3f sensorFusion::getEulerAnglesFromAccel(Vector3f accelBodyFrame)
{
	...
	//The code above uses conditional operator

	//float roll_accel = if (condition) is true ? do this : otherwise do this
