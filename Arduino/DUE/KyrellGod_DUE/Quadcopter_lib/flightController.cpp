...
void flightController::initTimeCritical()
{
	...
	// PWM resolution
	analogWriteResolution(12); ///this changes the PWM signal resolution from 8bit to 12bit
	///I guess this is for finer control?
	
	// Arduino related stuff that the subparts need
	///Used to control the motors
	_motors.motorsSetup(); ///motors is another class head over to motors.cpp to see this
	///Used to make some pretty sounds
	_buzzer.buzzerSetup(); ///buzzer is also another class
	_voltageControl.voltageControlSetup(); ///another class. This is used to measure the battery voltage
	
	// ESCs start up first
	_motors.sendMinSignal(); ///sends 1000 to motors so they don't spin
	
	// during this delay ESCs should confirm minimum pulse length
	delay(2000); ///ESCs have their own song they play so I think this gaurantees that they start up write. Hence why you send 1100
	
	releaseStabilizerOnMotors = false; ///after this routine go back to the Arduino code.
}

void flightController::playStartUpSound()
{
	_buzzer.startMelody(2, 100, 0, BUZZER_BLOCKING); ///Make two pulses each 100 ms
	_buzzer.startMelody(1, 500, 0, BUZZER_BLOCKING); ///make one long pulse at 500 ms
}
...
void flightController::startReceiverProcess()
{
	...
	while(!receiverModule::allChannelsInLimits())
	{
		...
		_buzzer.startMelody(1, 100, 1000, BUZZER_BLOCKING); ////This essentially will beep at you unless your receiver signals are 
		///being read properly
	}
...
void flightController::initSensors()
{
	...
	///This is an accelerometer - this must have a rate gyro as well
...
{
	///Yea just grab barometric pressure, accel and gyro
	accel = _mpu6050.readNpkgAccel();
	gyros = _mpu6050.readRpsGyro();
	magne = Vector3f(20.0f, 0.0f, -40.0f); //_hmc5883l.readCalibrated();
	//Barometer? This gets temperature pressure altitude
	//Altitude is calculated using some crazy equation
	//float altitute = ((pow((SEA_PRESSURE / pressure), 1/5.257) - 1.0) * (temperature + 273.15)) / 0.0065;
	//It would be interesting to try this on my drone.
	//As long as I print raw pressure I should be able to use this formula offline.
	//But can my sensor measure temp? Turns out mine can read temperature
	barom = _ms561101ba.getTPA(true);
}

void flightController::callSensorFusion()
{
  //This line performs all the sensor fusion 
	_sensorFusion.writeData(accel, gyros, magne, barom);

	//This just gets all the variables and puts it in the right vectors
	RPY_is = _sensorFusion.getRPY();
	RPYDot_is = _sensorFusion.getRPYDot();
	height_is = _sensorFusion.getHeight(); ///Awesome. Looks like it uses a barometer for height estimates
	heightDot_is = _sensorFusion.getHeightDot();
	heightDotDot_is = _sensorFusion.getHeightDotDot();
}
