...
stabilizer::stabilizer()
{
	...
	///roll_rate_command = 4.5*(roll_command-roll)
	///roll_out = 0.9*(roll_rate_command-roll_rate) + 1.0*integral(roll_rate_command-roll_rate)
...
{	
	...
	///this is partially right. I've dug a bit deeper
	//roll_rx hax been mapped from -30 to 30
	//the PID::compute routine takes (command,current)
	//it computes the error and multiplies it by kp
	//errorIntegral is a value that accrues over time using errorIntegral += error*dT where dT is 0.005 same as that loop timer.
	//since the loop timer is restricted by the scheduler to run at 200 Hz the dT remains correct.
	//I wonder if we can do the same and just use the elapsedTime variable
	...
	if(fabs(deadband<float>(yaw_rx, 5.0f)) > 0.0f)
	{
        ...
        #ifdef STABILIZER_BODY_RATES_CONVERSION //oh wait is this even on? Yes. Just verified in stabilizer.h
	...
	//This is the inverse of the H matrix going from Euler angle rates to pqr
	//But why are we converting RPYdot_is?
	//He's doing this because the RPYDot_is variable is actually in the Earth Frame.
	//Not sure why he does this conversion back and forth but it's worth rolling the quad
	//in multiple orientations to make sure we're getting the right thing.
	...
	//This converts the derivatives of Euler angle and converts them to body angular rates
	//This converts the commands to pqr which makes sens
	...
	//so roll_stab_output is in units of deg/s
	//notice though that the value here is constrained by +-500
	//The rate command here uses kp and ki
	...
	yaw_out = constrainn<float>(pidYaw[RATE].compute(yaw_stab_output, RPYDot_is_deg.z), -500.0f, 500.0f);
        #endif //end of BODY RATE COMPENSATION
	...
	//Turns out it doesn't work the way I have it. I'm going to try and code mine just like this
	//because mine isn't working. Granted I have different sensors and what not but it may be
	//that feeding roll_rate directly to the controller as derivative gain is too noisy.
	...
	//Add and substract from different motors but the limiters are things we could consider
	...
	//Yea see these are constrained to minimum spinning speed = 1100.
	...
	return Vector4f(pwm0, pwm1, pwm2, pwm3); //Then just return and send to motors
}
...
Vector3f stabilizer::getBodyRatesFromEulerRates(Vector3f eulerAngles, Vector3f eulerRatesDesired)
{
	...
	//Is this the H matrix inversed?
	//p,q,r = inv(H)*[phidot;thetadot;psidot]
	//Yea I just verified this with my dynamics textbook
	//So basically you take the output from the outerloop controller
	//and then convert it to body rates
