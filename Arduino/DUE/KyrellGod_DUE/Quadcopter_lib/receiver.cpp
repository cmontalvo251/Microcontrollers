...
#include <math.h> ////Built in library
#include <Arduino.h> ///Built in library 
#include "mathHelp.h" ////This will get some pi and other useful funcitons like mapp and constrainn
#include "receiver.h" ///This creates a couple #define variables and defines the 6 inputs and creates a receiver namespace
...
namespace receiverModule
{
	void init()
	{
		...
		pmc_set_writeprotect(false); ///I think this is built into the DUE code

		// enable external clock for counter TC2, channel 2
		pmc_enable_periph_clk(ID_TC8); //// I think this is too

		// set clock tp 84MHz/2 = 42MHz
		TC_Configure(TC2, 2, TC_CMR_TCCLKS_TIMER_CLOCK1); ///As well as this 

		// start timer
		TC_Start(TC2,2); ///And this...
		
		// connect pins and handlers
		///Ok so RECV_CHAN0PIN means if pin 44 changes run the function
		///ch0Handler which is defined below.
		attachInterrupt(RECV_CHAN0PIN, &ch0Handler, CHANGE); ///Might need to google this.
		attachInterrupt(RECV_CHAN1PIN, &ch1Handler, CHANGE); ///Just googled it. This is a built in 
		attachInterrupt(RECV_CHAN2PIN, &ch2Handler, CHANGE); ///function on the DUE that let's you handle interrupts
...
////This is pretty fancy. Basically ch0Handler is pwmHandler with the channel and the pin
...
///So everytime ch0-5Handler runs you will eventually run the pwmHanlder routine
///This function is pretty similar to the routine that I created but it just uses
///some fancy DUE functions 
///Basically it reads when the signal went high to low just like a normal PWM signal
