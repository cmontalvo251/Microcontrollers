...
void setup() 
{
  ...
  //  Serial.print("Sorry QUAD###.TXT has reached its limit. Suggest wiping SD card");
  //  while(1){};
  //}
  ...
  ///Make sure quad is on
  pulse_timeTrainer = pulseIn(inPin, HIGH,FREQUENCY);
  while(pulse_timeTrainer==0){
    Serial.print("Plug in Quad Battery ");
    pulse_timeTrainer = pulseIn(inPin, HIGH,FREQUENCY);
    Serial.println(pulse_timeTrainer);
    delay(1000);
  }

  ///Figure out what nominal signal is
  int NUMSIGNALS = 100;
  Serial.print("Reading HIGH Signals \n");
  int avg_signal = 0;
  for (int i = 0;i<NUMSIGNALS;i++){
    pulse_timeTrainer = pulseIn(inPin, HIGH,FREQUENCY);
    avg_signal+= pulse_timeTrainer;
    Serial.print(i);
    Serial.print(" ");
    Serial.println(pulse_timeTrainer);
  }
  avg_signal=avg_signal/NUMSIGNALS;
  Serial.print("Average High Signal = ");
  Serial.println(avg_signal);

  //Wait for pilot to switch to pin 1
  while (abs(pulse_timeTrainer-avg_signal) < 200) {
    Serial.println("Switch to Pin 1 \n");
    delay(1000);
    pulse_timeTrainer = pulseIn(inPin, HIGH,FREQUENCY);
  }

  ///Figure out what mid signal is
  Serial.print("Reading MID Signals \n");
  int mid_avg_signal = 0;
  for (int i = 0;i<NUMSIGNALS;i++){
    pulse_timeTrainer = pulseIn(inPin, HIGH,FREQUENCY);
    mid_avg_signal+= pulse_timeTrainer;
    Serial.print(i);
    Serial.print(" ");
    Serial.println(pulse_timeTrainer);
  }
  mid_avg_signal=mid_avg_signal/NUMSIGNALS;
  Serial.print("Average Mid Signal = ");
  Serial.println(mid_avg_signal);

  //Wait until pilot switch to pin 2
  while (abs(pulse_timeTrainer-mid_avg_signal) < 200) {
    Serial.println("Switch to Pin 2 \n");
    delay(1000);
    pulse_timeTrainer = pulseIn(inPin, HIGH,FREQUENCY);
  }

  ///Figure out what LOW signal is
  Serial.print("Reading LOW Signals \n");
  int low_avg_signal = 0;
  for (int i = 0;i<NUMSIGNALS;i++){
    pulse_timeTrainer = pulseIn(inPin, HIGH,FREQUENCY);
    low_avg_signal+= pulse_timeTrainer;
    Serial.print(i);
    Serial.print(" ");
    Serial.println(pulse_timeTrainer);
  }
  low_avg_signal=low_avg_signal/NUMSIGNALS;
  Serial.print("Average Low Signal = ");
  Serial.println(low_avg_signal);

  PULSELOW = (low_avg_signal + mid_avg_signal)/2;

  Serial.print("PULSELOW = ");
  Serial.println(PULSELOW);

  //Now wait for pilot to switch back to MID
  while (abs(pulse_timeTrainer-low_avg_signal) < 200) {
    Serial.println("Switch to back to manual \n");
    delay(1000);
    pulse_timeTrainer = pulseIn(inPin, HIGH,FREQUENCY);
  }
...
//  }
  ...
  if (autopilot) {
    ...
    //int nominal_val = 1500; //1560 //We tested this using cleanflight and it seems like that is correct.
