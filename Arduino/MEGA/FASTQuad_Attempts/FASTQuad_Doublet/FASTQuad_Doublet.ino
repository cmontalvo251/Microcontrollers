...
//  }
  ...
  if (autopilot) {
    ...
    if (autoOn != -99) {
      ...
      float timePulse = 0.1;
      float timePulse2 = 0.2;
      int Pulse = 100;
      if (diff < timePulse) {
        upper_left_command = nominal_val + Pulse;
        upper_right_command = nominal_val - Pulse;
        lower_left_command = nominal_val + Pulse;
        lower_right_command = nominal_val - Pulse;
      }
      else if ((diff >= timePulse) and (diff <= timePulse+timePulse2)) {
        upper_left_command = nominal_val - Pulse;
        upper_right_command = nominal_val + Pulse;
        lower_left_command = nominal_val - Pulse;
        lower_right_command = nominal_val + Pulse;
      } else {
        upper_left_command = ESCS_OFF;
        upper_right_command = ESCS_OFF;
        lower_left_command = ESCS_OFF;
        lower_right_command = ESCS_OFF;
      }
    }
