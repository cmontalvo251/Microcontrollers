...
// how long are max NMEA lines to parse? Software Serial can only handle 64
...
void Adafruit_GPS::common_init(void) {
  ...
  hour = minute = seconds = month = day =
    //fixquality = satellites = 0; // uint8_t
  // lat = lon = mag = 0; // char
  fix = false; // boolean
  //milliseconds = 0; // uint16_t
  latitude = longitude = altitude = 0.0;//speed = angle = 0.0; 
  // latitude = longitude = geoidheight = altitude =
  //   speed = angle = magvariation = HDOP = 0.0; // float
}

// void Adafruit_GPS::begin(uint16_t baud)
void Adafruit_GPS::begin()
{
#ifdef __AVR__
  if(gpsSwSerial) 
    gpsSwSerial->begin(9600); //hardcoded baud here
  // else 
